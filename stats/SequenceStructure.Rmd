...
sel.clus = which(clusters.table > 6)# at least 6 different sequences needed
...
for (i in seq(length(sel.clus))){
  ...
  schedules = get_schedules(sel.chunks[[i]], size)
...
for (i in seq(length(chosen_schedules))){
  ...
  json_file = paste0('../scheduling/sequences_', formatC(i, width = 3, flag = 0), '.json')

...
trialsfile = paste0('../data/trials_table-20190624_1706-Lund1db.csv')
trialsfilefmri = paste0('../data/trials_table-20190614_1549-Lund1fmridb.csv')
#trialsfile = paste0('../data/archive/trials_table-20190313_2239-Lundpilot2db.csv')
...
ntrials = trials.table %>% group_by(username, date_time, sess_num, run, sess_type) %>% summarise(n = max(trial, na.rm = T), cum_trial = sum(cumulative_trial == 1))
#interrupted_times = ntrials$date_time[ntrials$n < max(ntrials$n)]
...
# fmri
ntrials.fmri = trials.table.fmri %>% group_by(username, date_time, sess_num, run) %>% summarise(n = n(), cum_trial = sum(cumulative_trial == 1))
...
# Save clean data
```{r}
write.table(trials.table, file = '../data/archive/trials_table_Lund1_clean.csv')
write.table(trials.table.fmri, file = '../data/archive/trials_table_Lund1fmri_clean.csv')

```

# Number of sessions of each subject
```{r fig.width=25, fig.asp=0.7}
trials.sess = trials.table %>% group_by(username, sess_num, sess_date, sess_time) %>% summarise( n_sess = n() > 0) %>% 
  group_by(username, sess_num, sess_date) %>% summarise( n_sess = sum(n_sess))

plot.sess = ggplot(trials.sess, aes(x = sess_num, y = n_sess, group = username)) + geom_line(lwd=0.5) + geom_point(size=1.5) + facet_grid( username ~ .) + theme_lh() + theme(legend.position="none", axis.text.x = element_text(angle=90))

print(plot.sess)

trials.sess.fmri = trials.table.fmri %>% group_by(username, sess_num, sess_date, sess_time) %>% summarise( n_sess = n() > 0) %>% 
  group_by(username, sess_num, sess_date) %>% summarise( n_sess = sum(n_sess))
...
trials.table$group[grep("lue12", trials.table$username)] = "Control"
trials.table.fmri$group = "Experimental"
trials.table.fmri$group[grep("lue12", trials.table.fmri$username)] = "Control"
```
...
plot.MT = ggplot(ok_unpaced_trials, aes(x = trial, y = MT, col = as.factor(seq_train), group = true_sequence)) + geom_line(lwd=0.5) + geom_point(size=0.7) + ylim(miny, maxy) +  facet_grid( username ~ sess_num) + theme_lh() + theme(legend.position="none")

plot.MT.paced = ggplot(ok_paced_trials, aes(x = trial, y = MT, col = as.factor(seq_train), group = true_sequence)) + geom_line(lwd=0.5) + geom_point(size=0.7) + ylim(miny, maxy) + facet_grid( username ~ sess_num) + theme_lh() + theme(legend.position="none")
...
plot.meanMT = ggplot(subset(last_trials, group == "Experimental" & seq_train == "trained"), aes(x = sess_num, y = meanMT, group = true_sequence, col = as.factor(true_sequence))) + geom_line(lwd=1) + geom_point() + facet_grid( . ~ username ) + theme(legend.position="none") + theme_lh()
...
plot.meanMT.group = ggplot(last_trials.user, aes(x = sess_num, y = meanMT, group = username, col = as.factor(seq_train)))  + geom_vline(xintercept = test_sessions, lty = 2) +  geom_smooth(aes(group=as.factor(seq_train)))  + theme_lh() + theme( legend.position="none") + xlab('Session') + ylab('Movement duration mean (ms)') + facet_grid(.~group) 
...
plot.sdMT.group = ggplot(last_trials.user, aes(x = sess_num, y = sdMT, group = username, col = as.factor(seq_train))) + geom_vline(xintercept = test_sessions, lty = 2) +  geom_smooth(aes(group=as.factor(seq_train)))  + theme_lh() + theme( legend.position="none") + xlab('Session') + ylab('Movement duration SD (ms)') + facet_grid(.~group)
...
savings = merge(first.5, last.5, by = c("username", "sess_num", "true_sequence", "seq_train", "group"), suffix = c(".first", ".last")) %>% mutate(meanMT = (meanMT.first - meanMT.last)) %>%  group_by(username, sess_num, seq_train, group) %>% summarise(meanMT = mean(meanMT))
...
boxplot(value ~ type, data = distances.melt, ylab = "Correlation")

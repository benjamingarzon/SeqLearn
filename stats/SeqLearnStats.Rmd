...
suffix = '20181128_1606-KIpilot1db'
...
keys.table.median = keys.table.median %>% arrange(username, medianRT) %>% group_by(username) %>% mutate(rankRT = percent_rank(medianRT), transition= paste(key_from, key_to, sep = ' - '), size_trans = size_from + size_to)
...
keys.table.rank = keys.table.median %>% group_by(key_from, key_to, size_from, size_to, transition, size_trans) %>% summarise(meanrank = mean(rankRT), medianrank = median(rankRT), count = n(), minrank = min(rankRT), maxrank = max(rankRT)) %>% arrange(meanrank)
...
```{r}
trials.table = trials.table1
memo.table = memo.table1
keys.table = keys.table1
```
...
ok_unpaced_trials.group = ok_unpaced_trials %>% group_by(trial, sess_num, seq_train) %>% summarise(MT = mean(MT))
...
#trialsfile = paste0('../data/archive/trialsfile-kip1s006.csv')

...
username = 'benji'
keysfile = paste0('../data/archive/keysfile-', username, '.csv')
trialsfile = paste0('../data/archive/trialsfile-', username, '.csv')
...
rawdata = rawdata %>% mutate(choice = as.numeric(obs_sequence), condition = as.numeric(true_sequence), MT_sec = MT/1000)
...
for (cond in 1:num_cond) {
  for (choice in 1:num_choices) {
    ...
    choicedata[cond, 1, 1:tmp_trials] <- tmp$MT_sec # time in sec
...
dataList <- list(
  ...
  N_cond           = num_cond,
  N_tr             = nrow(rawdata),
  N_sess           = max(rawdata$sess_num),
  condition        = rawdata$condition,
  choice           = rawdata$choice,
  time             = rawdata$MT_sec, # in seconds!
  outcome          = rawdata$outcome, 
  conditionxchoice = conditionxchoice,
  sess_time        = rawdata$timediff,
  seq_trial        = rawdata$cumulative_trial,
  sess_num         = rawdata$sess_num
)


POI     <- c("alpha", "beta_reward", "beta_punishment", "Q0", "k", "A", "tau", "gamma", "forget", "Qt", "PEt", "time_rng", "choice_rng", "vt")
genInitList <- "random"

modelPath <- "./model/RL_lba_single.stan"
m = rstan::stan_model(modelPath)

 fit = rstan::vb(m,  output_samples = 1000,
                     data   = dataList,
                     #   = POI,
                     init   = genInitList)
...
traceplot(fit, par = c("alpha", "beta_reward","beta_punishment", "tau","gamma","k","A"))
#print(fit)
```
```{r}
...
#alpha0.samples <- inv.logit(parVals$logit_alpha0)
#logit_alpha1.samples <- parVals$logit_alpha1
#f0.samples <- parVals$f0 
#f1.samples <- parVals$f1 
forget.samples <- parVals$forget 
...
#Q0_on = apply(Q0_on.samples, 2, median)
#Q0_off = apply(Q0_off.samples, 2, median)
vt = apply(vt.samples, c(2,3), median)
#forget = apply(forget.samples, 2, median)
forget = median(forget.samples)
#f0 = median(f0.samples)
#f1 = median(f1.samples)
Q0 = apply(Q0.samples, c(2,3), median)
#colnames(Q0) = choice_labels
#rownames(Q0) = condition_labels
Qt = apply(Qt.samples, c(2,3), median)
...
#rawdata$forget = forget

print(ggplot(rawdata, aes(x = cumulative_trial, y = choice_rng, col = as.factor(correct))) + geom_point() + facet_grid(true_sequence ~ sess_num) + theme(legend.position = "bottom"))
print(ggplot(rawdata, aes(x = cumulative_trial, y = PEt)) + geom_line() + facet_grid(true_sequence ~ sess_num))
#print(ggplot(rawdata, aes(x = cumulative_trial, y = forget)) + geom_line() + facet_grid(true_sequence ~ sess_num))

print(ggplot(rawdata, aes(x = cumulative_trial, y = time_rng)) + geom_line() + facet_grid(true_sequence ~ sess_num))

print(ggplot(rawdata, aes(x = time_rng)) + geom_density() + facet_grid(true_sequence ~ sess_num))
print(ggplot(rawdata, aes(x = MT_sec)) + geom_density() + facet_grid(true_sequence ~ sess_num))
